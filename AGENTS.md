...
- Sequencing: 広い依頼は `secretary-router` を先に使い、最新性が重要なら `trend-research` を企画前に使い、何を作るかを固めるときは `product-planning`、実装前は `gstack-plan-eng-review`、実装は `builder`、マージ前は `gstack-review`、最後の実行と引き渡しは `gstack-ship` を使う。
